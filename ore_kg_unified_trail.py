...
def compute_candidate_feature_maps(candidate_pool, cluster_heads, cluster_neigh_lookup, results, bm25_scores, laff_graph):
    bm25_raw = {}
    neigh_raw = {}
    utility_raw = {}
    ...
    exploit_strength = float(np.clip(0.55 * frontier_ratio + 0.45 * bm25_nonzero_ratio, 0.20, 0.85))
...
class OREAdaptiveKGUnionOnBaseline(pt.Transformer):
    ...
    # ── CHANGE 1: Return bm25_map alongside candidates ──────────────────────
    def _get_union_expansion_neighbors(self, qid: str, query: str, docno: str, neighbors: np.ndarray, weights: np.ndarray):
        ...
        if len(candidates) == 0:
            return laff_top, [], {}  # ── CHANGE 1: empty bm25_map
        ...
        if len(candidates) == 0:
            return laff_top, [], bm25_map  # ── CHANGE 1: return bm25_map anyway
        ...
        # ── CHANGE 1: return bm25_map as third element ──
        return laff_top, [(n, rrf, comp, 0.0) for n, rrf, comp, _ in kg_candidates], bm25_map
        ...
        for _, (query, initial_results) in enumerate(groups):
            ...
            while len(arms) > 0 and len(results) < self.budget:
                if count == 0:
                    ...
                    if len(bm25_scores) < (len(initial_results) + self.num_bm25_calls):
                        ...
                        if len(donos_missing) > 0:
                            ...
                            bm25_scores.update(dict(zip(docnos, scores)))

                    # ── CHANGE 3: BM25 fallback for expansion docs ──────────
                    # Build the fallback pool BEFORE entering the CER branches.
                    # Only expansion docs (kg/laff) that have bm25 > 0 but are
                    # NOT in cluster_neigh_lookup.  This doesn't touch baseline
                    # BM25 docs (source='bm25'), preserving baseline CER behaviour.
                    expansion_fallback = [
                        (a, bm25_scores.get(a.docnos[-1], 0.0))
                        for a in filtered_arms
                        if a.docnos[-1] not in cluster_neigh_lookup
                        and doc_source.get(str(a.docnos[-1]), '') in ('kg', 'laff')
                        and bm25_scores.get(a.docnos[-1], 0.0) > 0.0
                    ]
                    expansion_fallback_arms = [
                        a for a, _ in heapq.nlargest(
                            10, expansion_fallback, key=lambda x: x[1]
                        )
                    ]
                    expansion_fallback_set = {a.docnos[-1] for a in expansion_fallback_arms}
                    # ── END CHANGE 3 (pool built) ───────────────────────────
                    ...
                    if prev_heads == cluster_heads:
                        ...
                        remaining_arms = [(arm, bm25_scores.get(arm.docnos[-1], 0)) for arm in neighbor_criteria_arms if arm.docnos[-1] in bm25_scores and arm.docnos[-1] not in selected_docnos]
                        bm25_arms = [arm for arm, _ in heapq.nlargest(25, remaining_arms, key=lambda x: x[1])]

                        new_arms.extend(bm25_arms)

                        # ── CHANGE 3: append expansion fallback ─────────────
                        selected_so_far = {a.docnos[-1] for a in new_arms}
                        for a in expansion_fallback_arms:
                            if a.docnos[-1] not in selected_so_far:
                                new_arms.append(a)
                        # ── END CHANGE 3 ────────────────────────────────────
                        ...
                        feature_maps = compute_candidate_feature_maps(
                            new_arms, cluster_heads, cluster_neigh_lookup, results, bm25_scores, self.laff_graph
                        )
                        for cand in new_arms:
                            d = cand.docnos[-1]
                        ...
                        # ── CHANGE 3: append expansion fallback ─────────────
                        for a in expansion_fallback_arms:
                            if a.docnos[-1] not in {x.docnos[-1] for x in new_arms}:
                                new_arms.add(a)
                        # ── END CHANGE 3 ────────────────────────────────────
                ...
                if len(results) < self.budget:
                    ...
                    # ── CHANGE 2: collect BM25 scores from KG expansion ─────
                    expansion_bm25_scores = {}
                    # ── END CHANGE 2 (init) ─────────────────────────────────
                    ...
                    for docno in neighbor_lookup:
                        ...
                        # ── CHANGE 1: unpack third return value ─────────────
                        laff_neighbors, kg_candidates, parent_bm25_map = self._get_union_expansion_neighbors(qid, query, docno, neighbors, weights)
                        expansion_bm25_scores.update(parent_bm25_map)
                        # ── END CHANGE 1 ────────────────────────────────────
                    ...
                    # ── CHANGE 2: batch BM25 for new LAFF docs & inject ─────
                    new_laff_docnos = [
                        n for n, _, _ in laff_to_add
                        if n not in all_docnos and n not in bm25_scores
                    ]
                    # deduplicate
                    seen = set()
                    new_laff_unique = []
                    for n in new_laff_docnos:
                        if n not in seen:
                            seen.add(n)
                            new_laff_unique.append(n)
                    new_laff_docnos = new_laff_unique

                    if len(new_laff_docnos) > 0:
                        laff_bm25_batch = pd.DataFrame([
                            {'qid': qid, 'query': query, 'docno': n,
                             'text': docstore.get(n).text}
                            for n in new_laff_docnos
                        ])
                        laff_bm25_result = ret_scorer(laff_bm25_batch)
                        expansion_bm25_scores.update(
                            dict(zip(laff_bm25_result['docno'],
                                     laff_bm25_result['score']))
                        )

                    # Inject all expansion BM25 scores (only for docs not
                    # already in bm25_scores — don't overwrite BM25 pool scores)
                    for d, s in expansion_bm25_scores.items():
                        if d not in bm25_scores:
                            bm25_scores[d] = s
                    # ── END CHANGE 2 ────────────────────────────────────────
            ...
            final_topk = [doc for doc, _score in Counter(results).most_common(self.budget)]
            final_rel = sum(1 for d in final_topk if self._get_qrel_label(qid, d) >= 2)
            total_rel = len(self.qrels_map.get(str(qid), set()))
            recall = (final_rel / total_rel) if total_rel > 0 else 0.0

            print(
                f"[QUERY] qid={qid} top{self.budget}_rel={final_rel}/{total_rel} recall@{self.budget}={recall:.4f} "
                f"pool: bm25={pool_by_source.get('bm25',0)} laff={pool_by_source.get('laff',0)} kg={pool_by_source.get('kg',0)}"
            )
...
class ArmBaselineUnion:
    ...
    def estimate_cer_score(self, doc, feature_maps, lambda_param, lambda_1, lambda_2):
        ...
        cer_score = (lambda_param * bm25_norm) + (lambda_1 * neigh_norm) + (lambda_2 * utility_mix)
