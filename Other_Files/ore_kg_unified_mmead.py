...
from kg_scorer_unified import KGScorerUnified, create_scorer
...
class OREAdaptiveKGUnified(pt.Transformer):
    ...
    def _get_kg_enhanced_neighbors(
        ...
        """
        Get KG-enhanced neighbor scores.
        No doc text needed - entities come from KGPR's EL.
        """
        neighbor_docnos = [str(n) for n in neighbors]
        rescored = self.kg_scorer.rescore_neighbors(
            docno, neighbor_docnos, weights
        )
        
       # return [(docno, score) for docno, score, _ in rescored]
        return [(neighbor_docno, score) for neighbor_docno, score, _ in rescored]
    
    def _compute_kg_enhanced_cluster_lookup(self, cluster_heads):
        cluster_neigh_lookup = defaultdict(list)
        
        for cluster_head in cluster_heads:
            neighbors, weights = self.laff_graph.neighbours(cluster_head, weights=True)
            kg_scores = self._get_kg_enhanced_neighbors(cluster_head, neighbors, weights)
        ...
        for i, (query, initial_results) in enumerate(groups):
            ...
            while len(arms) > 0 and len(results) < self.budget:
                if count == 0:
                    ...
                    cluster_neigh_lookup = self._compute_kg_enhanced_cluster_lookup(cluster_heads)
                ...
                if len(results) < self.budget:
                    ...
                    for docno in neighbor_lookup:
                        ...
                        kg_scores = self._get_kg_enhanced_neighbors(docno, neighbors, weights)
...
class ArmKG:
    ...
    def estimate_cer_score(
        ...
        final_score = self.estimated_scores + lambda_2 * score_utility

        self.cer_scores[docnos[-1]] = final_score
        return final_score
...
def create_ore_kg(
    ...
    # MMEAD options
    use_mmead: bool = False,
    mmead_linker: str = 'rel',
    mmead_emb_dim: int = 300,
    mmead_cache: str = None,
    # ORE parameters
    **kwargs
):
    """
    ...
    If use_mmead=True, uses MMEAD (Wikipedia2Vec) instead of KGPR + Freebase.
    If mmead_cache is provided, uses pre-built SQLite cache for speed.
    """
    if use_mmead:
        from mmead_scorer import create_mmead_scorer
        kg_scorer = create_mmead_scorer(
            alpha=kg_alpha,
            beta=kg_beta,
            gamma=kg_gamma,
            emb_dim=mmead_emb_dim,
            linker=mmead_linker,
            cache_db=mmead_cache,
        )
    else:
