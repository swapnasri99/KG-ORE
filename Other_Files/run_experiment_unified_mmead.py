...
# MMEAD options
parser.add_argument("--use_mmead", action="store_true", help="Use MMEAD instead of KGPR + Freebase")
parser.add_argument("--mmead_linker", type=str, default="rel", choices=["rel", "blink"])
parser.add_argument("--mmead_emb_dim", type=int, default=300, choices=[300, 500])
parser.add_argument("--mmead_cache", type=str, default=None, help="Path to mmead_cache.db (FAST mode)")

args = parser.parse_args()

# Validate
if not args.use_mmead and not args.passage_el and not args.passage_el_db:
    print("ERROR: Provide --use_mmead, --passage_el (JSONL), or --passage_el_db (SQLite)")
    exit(1)

if not args.use_mmead and args.gamma > 0 and not args.freebase_dir:
    print("ERROR: --gamma > 0 requires --freebase_dir (or use --use_mmead)")
    exit(1)
...
from ore_kg_unified_mmead import create_ore_kg
...
print(f"  MMEAD: {'ENABLED' if args.use_mmead else 'DISABLED'}")
if args.use_mmead and args.mmead_cache:
    print(f"  MMEAD Cache: {args.mmead_cache}")
...
ore_kg = create_ore_kg(
    ...
    # MMEAD
    use_mmead=args.use_mmead,
    mmead_linker=args.mmead_linker,
    mmead_emb_dim=args.mmead_emb_dim,
    mmead_cache=args.mmead_cache,
    # ORE
    budget=args.budget,
