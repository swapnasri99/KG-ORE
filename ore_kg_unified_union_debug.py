...
class OREAdaptiveKGUnified(pt.Transformer):
    ...
    def _get_union_neighbors(
        ...
        """
        Union strategy: take set union of raw LAFF top-k and KG-rescored top-k.

        Returns:
            union_rescored: list of (neighbor_docno, combined_score, comp)
            source_map: dict neighbor_docno -> {'kg_only', 'laff_only', 'both'}
        """
        neighbor_docnos = [str(n) for n in neighbors]
    ...
    def transform(self, inp: pd.DataFrame) -> pd.DataFrame:
        ...
        for _, (query, initial_results) in enumerate(groups):
            ...
            kg_only_docs = [
                (rank, d) for rank, d in enumerate(top50, start=1)
                if doc_source.get(d) == 'kg_only'
            ]
            laff_only_docs = [
                (rank, d) for rank, d in enumerate(top50, start=1)
                if doc_source.get(d) == 'laff_only'
            ]
            both_docs = [
                (rank, d) for rank, d in enumerate(top50, start=1)
                if doc_source.get(d) == 'both'
            ]

            relevant_kg_only_docs = [
                (rank, d) for rank, d in kg_only_docs
                if d in relevant_docs
            ]

            print('\n' + '=' * 70)
            print(f'[KG TOP50 DEBUG] qid={qid}')
            print(f'Final top-50 docs: {len(top50)}')
            print(f'KG-only docs in top-50: {len(kg_only_docs)}')
            print(f'LAFF-only docs in top-50: {len(laff_only_docs)}')
            print(f'Both(KG and LAFF) docs in top-50: {len(both_docs)}')
            print(f'Relevant KG-only docs in top-50 (rel>=2): {len(relevant_kg_only_docs)}')

            if len(kg_only_docs) > 0:
                print(f'KG-only precision@top50: {len(relevant_kg_only_docs) / len(kg_only_docs):.3f}')
                print('KG-only docs captured in final top-50:')
                for rank, d in kg_only_docs:
                    rel_flag = 'REL' if d in relevant_docs else 'NOT_REL'
                    print(f'  rank={rank:2d}  docno={d}  {rel_flag}')

            print('Top-50 provenance + relevance:')
