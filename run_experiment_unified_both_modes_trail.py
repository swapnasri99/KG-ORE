...
parser.add_argument('--qid', type=str, default=None, help='Run only this specific qid')
...
if args.skip_queries > 0 or args.max_queries is not None:
    ...
    print(f'Running {len(selected_qids)} queries (skip={args.skip_queries})')

if args.qid is not None:
    topics_df = topics_df[topics_df['qid'] == args.qid].copy()
    qrels_df = qrels_df[qrels_df['qid'] == args.qid].copy()
    print(f'Filtering to single qid={args.qid}')


qrels_map = qrels_df[qrels_df['label'] >= 2].groupby('qid')['docno'].apply(set).to_dict()
print(f'Loaded qrels for {len(qrels_map)} queries (rel>=2).')
print('All components loaded\n')

from ore_kg_unified_trail import create_ore_kg_union_on_baseline
