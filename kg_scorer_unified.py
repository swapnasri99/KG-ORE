...
# ============================================================
# Entity Store (loads KGPR's EL results)
# ============================================================
    ...
    def _load_el_file(self, path: str, storage: dict, label: str):
        ...
        with open(path, 'r', encoding='utf-8') as f:
            for line in tqdm(f, desc=f"  Loading {label} EL", mininterval=2.0):
                ...
                # Skip if already loaded (small file takes priority)
...
class FreebaseGraph:
    ...
    def __init__(self, freebase_dir: str):
        ...
        # Build sparse adjacency (both directions)
...
class KGScorerUnified:
    ...
    VALID_KG_MODES = {'binary', 'count', 'log', 'coverage', 'ratio'}
    ...
    def rescore_neighbors(
        ...
        NOTE: No doc_text needed! Entities come from KGPR's pre-computed EL.
        
        Args:
            docno: Source document ID
            neighbor_docnos: List of neighbor document IDs
            laff_weights: Array of LAFF weights
        
        Returns:
            List of (neighbor_docno, combined_score, components) sorted DESC
        """
        # Source document entities (from KGPR)
...
def create_scorer(
    ...
    **kwargs  # Accept extra kwargs for compatibility
    ...
    Examples:
        # Entity overlap only (no Freebase needed)
        scorer = create_scorer(alpha=0.5, beta=0.5, gamma=0.0,
                               passage_el_path="kgpr_data/.../passage_test_with_id_bm25rank1000.jsonl")
        
        # Full KG scoring
        scorer = create_scorer(alpha=0.4, beta=0.3, gamma=0.3,
                               freebase_dir="kgpr_data/freebase",
                               passage_el_path="kgpr_data/.../msmarco_passage_with_id.jsonl")
    """
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        ...
        print("  python kg_scorer_unified.py kgpr_data/entity_linking_results/passage_test_with_id_bm25rank1000.jsonl")
        print("\n  # With KG connectivity:")
        print("  python kg_scorer_unified.py kgpr_data/entity_linking_results/passage_test_with_id_bm25rank1000.jsonl kgpr_data/freebase")
        sys.exit(0)
    ...
    # Test with some sample passages
