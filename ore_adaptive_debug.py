    ...
    def transform(self, inp: pd.DataFrame) -> pd.DataFrame:
        ...
        for _, (query, initial_results) in enumerate(groups):
                ...
                if count == 0:
                    ...
                    if prev_heads == cluster_heads:
                        ...
                        arm = sorted(
                            ...
                            if x.docnos[-1] in list(x.cer_scores.keys())
                ...
                if len(results) < (min(self.batch_size * self.cross_enc_budget, self.budget)):
                    ...
                    if count > 0:
                        bm25_features = np.array([
                            x.bm25_scores[x.docnos[-1]] if x.docnos[-1] in x.bm25_scores else 0 for x in arm
                        ]).reshape(-1, 1)
            ...
            if self.verbose:
                ...
                top50_rel = sum(1 for d in top50 if d in relevant_docs)

                print('\n' + '=' * 58)
                print(f'[TOP50_REL_TOTAL] qid={qid}')
                print(f'relevant_in_top50: {top50_rel}')
                print('=' * 58)
